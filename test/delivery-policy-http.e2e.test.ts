...
async function platformFixture() {
  const userId = randomUUID(), token = randomUUID(); users.push(userId);
  await owner.query('INSERT INTO users (id,display_name,updated_at) VALUES ($1,$2,now())', [userId, 'Policy platform administrator']);
  await owner.query("INSERT INTO platform_role_assignments (id,user_id,role,granted_by) VALUES ($1,$2,'platform_administrator',$2)", [randomUUID(), userId]);
  await owner.query("INSERT INTO mfa_factors (id,user_id,type,encrypted_secret,enabled_at) VALUES ($1,$2,'totp','policy-platform',now())", [randomUUID(), userId]);
  await owner.query(`INSERT INTO sessions (id,user_id,access_token_hash,refresh_token_hash,authentication_method,device_id,access_expires_at,expires_at,last_seen_at)
    VALUES ($1,$2,$3,$4,'administrator_mfa','policy-platform',now()+interval '1 hour',now()+interval '1 day',now())`, [randomUUID(), userId, hash(token), hash(randomUUID())]);
  return token;
}
...
after(async () => { await app?.close(); await owner.query('DELETE FROM audit_events WHERE vendor_id=ANY($1::uuid[])', [vendors]); await owner.query('DELETE FROM vendor_memberships WHERE vendor_id=ANY($1::uuid[])', [vendors]); await owner.query('DELETE FROM sessions WHERE user_id=ANY($1::uuid[])', [users]); await owner.query('DELETE FROM mfa_factors WHERE user_id=ANY($1::uuid[])', [users]); await owner.query('DELETE FROM platform_role_assignments WHERE user_id=ANY($1::uuid[])', [users]); await owner.query('DELETE FROM users WHERE id=ANY($1::uuid[])', [users]); await owner.query('DELETE FROM vendors WHERE id=ANY($1::uuid[])', [vendors]); await owner.end(); });

void test('delivery policy HTTP contract validates, authorizes, versions, and audits safe policy values', async () => {
  const current = await fixture('vendor_owner'), admin = await fixture('vendor_administrator'), agent = await fixture('delivery_agent'), customer = await fixture('customer'), platformToken = await platformFixture(); const path = `/v1/vendors/${current.vendorId}/delivery-policy`;
  ...
  for (const body of [
    ...
    { skipCutoffMinutes: 60, lateLeavePolicy: 'approval', captureAgentLocationEvidence: false, expectedVersion: 1, reason: '   ' },
    { skipCutoffMinutes: 60, lateLeavePolicy: 'approval', captureAgentLocationEvidence: false, expectedVersion: 1, reason: 'x'.repeat(501) },
  ...
  await error(await api(path, platformToken), 403, 'FORBIDDEN');
  ...
  assert.equal((await api(`/v1/vendors/${admin.vendorId}/delivery-policy`, admin.token, { method: 'PATCH', body: { skipCutoffMinutes: 10080, lateLeavePolicy: 'approval', captureAgentLocationEvidence: false, expectedVersion: 1, reason: ` ${'x'.repeat(500)} ` } })).status, 200);
