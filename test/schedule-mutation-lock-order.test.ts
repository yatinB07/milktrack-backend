...
void test('subscription creation locks only applicable horizon weekdays before configuration reads', async () => {
  ...
  const lock = { lock: (_tx: TransactionContext, _vendorId: string, dates: string[]) => { calls.push(`dates:${dates.join(',')}`); return Promise.resolve(); } };
  const service = new DefaultSubscriptionService(authorization as never, { create: () => { calls.push('create'); return Promise.resolve(aggregate); } } as never, { requireSubscriptionSelection: () => { calls.push('catalog'); return Promise.resolve({ unitDecimalScale: 0 }); } } as never, { requireSubscriptionHousehold: () => { calls.push('household'); return Promise.resolve({}); } } as never, timezone as never, { append: () => Promise.resolve() }, lock, regeneration);
