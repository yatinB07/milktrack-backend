...
void test('route lifecycle acquires the ascending seven-day schedule horizon before its aggregate lock', async () => {
  ...
  const service = new DefaultRouteService(authorization as never, routes as never, {} as never, {} as never, { requireRouteDeliverySlot: () => { calls.push('slot'); return Promise.resolve({}); } } as never, {} as never, {} as never, timezone as never, { append: () => Promise.resolve() }, lock);
...
void test('subscription creation locks only applicable horizon weekdays before configuration reads', async () => {
  ...
  const lock = { lock: (_tx: TransactionContext, _vendorId: string, dates: string[]) => { calls.push(`dates:${dates.join(',')}`); return Promise.resolve(); } };
  const service = new DefaultSubscriptionService(authorization as never, { create: () => { calls.push('create'); return Promise.resolve(aggregate); } } as never, { requireSubscriptionSelection: () => { calls.push('catalog'); return Promise.resolve({ unitDecimalScale: 0 }); } } as never, { requireSubscriptionHousehold: () => { calls.push('household'); return Promise.resolve({}); } } as never, timezone as never, { append: () => Promise.resolve() }, lock);
