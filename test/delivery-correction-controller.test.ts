...
import { CorrectDeliveryRequestDto } from '../src/delivery/http/delivery.dto.js';
...
void test('vendor correction route forwards its validated command and maps the authoritative detail', async () => {
  ...
  const body = plainToInstance(CorrectDeliveryRequestDto, { expectedVersion: 2, replacementOutcome: 'delivered', actualQuantity: '1', reason: 'Verified route sheet' });
...
void test('correction request requires an exact replacement status, a positive quantity format, and a trimmed reason', async () => {
  ...
  ]) assert.ok((await validate(plainToInstance(CorrectDeliveryRequestDto, input))).length > 0);
});
