...
void test('delivery correction remains a vendor-only append route', () => {
  const correct = Object.getOwnPropertyDescriptor(VendorDeliveryController.prototype, 'correct')?.value as object;
  assert.equal(Reflect.getMetadata('path', VendorDeliveryController), 'vendors/:vendorId/deliveries');
  assert.equal(Reflect.getMetadata('path', correct), ':scheduledDeliveryId/corrections');
