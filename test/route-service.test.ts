...
void test('reactivation locks and version-checks the route before validating its immutable slot', async () => {
  const order: string[] = [];
  const routes = {
  ...
  const service = new DefaultRouteService(authorization as never, routes as never, catalog as never, audits);
...
void test('create normalizes route identity and audits only the safe persisted projection', async () => {
  ...
  const service = new DefaultRouteService(authorization as never, routes as never, catalog as never, audit);
