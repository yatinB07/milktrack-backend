...
void test('replace validates vendor-local date, locks route and slot, validates households in order, persists, and audits',async()=>{
  ...
  const households={
    requireRouteHouseholdSummaries:(_tx:TransactionContext,ids:readonly string[])=>{calls.push(`households:${ids.join(',')}`);return Promise.resolve(ids.map((id)=>household(id)));},
  };
  ...
  assert.deepEqual(result,{...projection,stops:[{...projection.stops[0],household:household('a')}]});assert.deepEqual(calls,['timezone','route','slot','households:a,b','replace:b,a']);
