...
async function fixture() {
  ...
  const value = {
    ...
    lateDeliveryId: randomUUID(), leaveDeliveryId: randomUUID(), deliveredId: randomUUID(), skippedId: randomUUID(),
    overrideId: randomUUID(), globalPriceId: randomUUID(), lateDate, onTimeDate, skipDate,
  };
  ...
  try {
    ...
    for (const weekday of new Set([DateTime.fromISO(value.lateDate).weekday, DateTime.fromISO(value.onTimeDate).weekday])) {
...
void test('public HTTP completes mixed leave, atomic delivery, correction, privacy, and tenant-denial flow', async () => {
  ...
  try {
    ...
    assert.deepEqual((await owner.query<{ id: string; status: string }>('SELECT id,status FROM scheduled_deliveries WHERE id=ANY($1::uuid[]) ORDER BY id', [[value.leaveDeliveryId, value.deliveredId]])).rows,
      [{ id: value.leaveDeliveryId, status: 'skipped_by_customer' }, { id: value.deliveredId, status: 'scheduled' }].sort((a, b) => a.id.localeCompare(b.id)));
    ...
    const lateDecision = (await owner.query<{ id: string; version: number }>("SELECT id,version FROM leave_occurrence_decisions WHERE vendor_id=$1 AND service_date=$2 AND status='pending'", [value.vendorId, value.lateDate])).rows[0];
    assert.ok(lateDecision);
    await json(await api(`/vendors/${value.vendorId}/leave-occurrence-decisions/${lateDecision.id}/decision`, value.ownerToken, {
    ...
    const blocked = schedule.items.find(({ id }) => id === value.leaveDeliveryId); const actionable = schedule.items.find(({ id }) => id === value.deliveredId);
    assert.equal(blocked?.blockedByCustomerLeave, true); assert(!blocked?.pendingStopItems.some(({ scheduledDeliveryId }) => scheduledDeliveryId === value.leaveDeliveryId));
    assert.ok(actionable?.pendingStopItems.some(({ scheduledDeliveryId }) => scheduledDeliveryId === value.deliveredId));

    const eventCount = Number((await owner.query<{ count: string }>('SELECT count(*) count FROM delivery_events WHERE scheduled_delivery_id=$1', [value.leaveDeliveryId])).rows[0]?.count);
    const blockedAttempt = await api(`/agent/vendors/${value.vendorId}/route-stops/${value.stopId}/outcomes`, value.agentToken, {
      method: 'POST', body: { serviceDate: value.onTimeDate, outcome: 'delivered', occurredAt: new Date().toISOString(), items: [{ scheduledDeliveryId: value.leaveDeliveryId, expectedVersion: 2, actualQuantity: '1' }] },
    });
    assert.equal(blockedAttempt.status, 409); assert.equal(Number((await owner.query<{ count: string }>('SELECT count(*) count FROM delivery_events WHERE scheduled_delivery_id=$1', [value.leaveDeliveryId])).rows[0]?.count), eventCount);
    assert.equal((await owner.query('SELECT 1 FROM delivery_price_snapshots WHERE scheduled_delivery_id=$1', [value.leaveDeliveryId])).rowCount, 0);

    const occurredAt = new Date(Date.now() - 1000).toISOString();
    const delivered = await json<{ items: Array<{ currentStatus: string; version: number }> }>(await api(`/agent/vendors/${value.vendorId}/route-stops/${value.stopId}/outcomes`, value.agentToken, {
      method: 'POST', body: { serviceDate: value.onTimeDate, outcome: 'delivered', occurredAt, items: [{ scheduledDeliveryId: value.deliveredId, expectedVersion: 1, actualQuantity: '1.500' }] },
    }), 201);
    assert.deepEqual(delivered.items.map(({ currentStatus, version }) => ({ currentStatus, version })), [{ currentStatus: 'delivered', version: 2 }]);
    ...
    const notifications = await json<{ items: Array<{ type: string; payload: Record<string, string> }> }>(await api(`${customerBase}/notifications`, value.customerToken), 200);
    ...
    assert.equal((await owner.query<{ count: number }>('SELECT count(*)::int count FROM delivery_price_snapshots WHERE vendor_id=$1', [value.vendorId])).rows[0]?.count, 1);
