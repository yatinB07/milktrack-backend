...
import { CustomerDeliveryDetailResponseDto, CustomerDeliveryListResponseDto, DeliveryPageQueryDto, toCustomerDeliveryDetailResponse, toDeliverySummaryResponse } from './delivery.dto.js';
...
export class CustomerDeliveryController {
  ...
  @Get() @ApiResponse({ status: 200, type: CustomerDeliveryListResponseDto }) async list(@Param('vendorId', new ParseUUIDPipe({ version: '4' })) vendorId: string, @Param('householdId', new ParseUUIDPipe({ version: '4' })) householdId: string, @Query() query: DeliveryPageQueryDto) {
  ...
  @Get(':scheduledDeliveryId') @ApiResponse({ status: 200, type: CustomerDeliveryDetailResponseDto }) get(@Param('vendorId', new ParseUUIDPipe({ version: '4' })) vendorId: string, @Param('householdId', new ParseUUIDPipe({ version: '4' })) householdId: string, @Param('scheduledDeliveryId', new ParseUUIDPipe({ version: '4' })) scheduledDeliveryId: string) {
    return this.deliveries.getCustomerDetail(requestContextStore.requireActor(), vendorId, householdId, scheduledDeliveryId).then(toCustomerDeliveryDetailResponse);
  }
}
