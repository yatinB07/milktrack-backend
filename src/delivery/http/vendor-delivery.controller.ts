...
import { CorrectDeliveryRequestDto, VendorDeliveryListResponseDto, VendorDeliveryDetailResponseDto, VendorDeliveryPageQueryDto, toDeliverySummaryResponse, toVendorDeliveryDetailResponse } from './delivery.dto.js';
...
export class VendorDeliveryController {
  ...
  @Get() @ApiResponse({ status: 200, type: VendorDeliveryListResponseDto }) async list(@Param('vendorId', new ParseUUIDPipe({ version: '4' })) vendorId: string, @Query() query: VendorDeliveryPageQueryDto) {
  ...
  @Get(':scheduledDeliveryId') @ApiResponse({ status: 200, type: VendorDeliveryDetailResponseDto }) get(@Param('vendorId', new ParseUUIDPipe({ version: '4' })) vendorId: string, @Param('scheduledDeliveryId', new ParseUUIDPipe({ version: '4' })) scheduledDeliveryId: string) {
    return this.deliveries.getVendorDetail(requestContextStore.requireActor(), vendorId, scheduledDeliveryId).then(toVendorDeliveryDetailResponse);
  }

  @Post(':scheduledDeliveryId/corrections') @HttpCode(200) @ApiResponse({ status: 200, type: VendorDeliveryDetailResponseDto }) correct(@Param('vendorId', new ParseUUIDPipe({ version: '4' })) vendorId: string, @Param('scheduledDeliveryId', new ParseUUIDPipe({ version: '4' })) scheduledDeliveryId: string, @Body() body: CorrectDeliveryRequestDto) {
