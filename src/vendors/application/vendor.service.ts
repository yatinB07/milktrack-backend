...
export abstract class VendorService {
  ...
  abstract getDeliveryPolicyForTransaction(tx: TransactionContext, vendorId: string): Promise<DeliveryPolicy>;
...
export class PrismaVendorService extends VendorService {
  ...
  getDeliveryPolicyForTransaction(tx: TransactionContext, vendorId: string) {
    return this.vendors.getDeliveryPolicy(tx, vendorId);
  }
  ...
  updateDeliveryPolicy(actor: Actor, vendorId: string, command: UpdateDeliveryPolicyCommand): Promise<DeliveryPolicy> {
    return this.tenantAuthorization.execute({ actor, vendorId, permission: 'schedule:manage', operation: 'vendor.delivery-policy.update' }, async (tx) => {
