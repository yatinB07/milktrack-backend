...
const date = (value: string) => {
  ...
  if (!parsed.isValid || parsed.toFormat('yyyy-MM-dd') !== value)
    throw error('INVALID_ROUTE_STOP_DATE', 'Route stop date must be a valid ISO calendar date');
  return parsed;
};
...
export function normalizeRouteStopReplacement(
  ...
  if (effective < date(today))
    throw error('INVALID_ROUTE_STOP_DATE', 'Route stop date cannot be in the past');
  if (new Set(householdIds).size !== householdIds.length)
    throw error('DUPLICATE_ROUTE_STOP', 'A household can appear only once in a route stop plan');
