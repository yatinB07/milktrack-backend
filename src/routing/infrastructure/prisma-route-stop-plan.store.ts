...
import { RouteStopPlanStore, type ReplaceRouteStopsInput } from '../application/route-stop-plan.store.js';
...
export class PrismaRouteStopPlanStore extends RouteStopPlanStore {
  async list(context: TransactionContext, route: RouteRecord, serviceDate: string) {
    ...
    const plans = await tx.$queryRaw<PlanRow[]>(Prisma.sql`
      ...
      FROM route_stop_plans
      WHERE route_id=${route.id}::uuid AND superseded_at IS NULL
        AND effective_from <= ${serviceDate}::date
        AND (effective_to IS NULL OR effective_to > ${serviceDate}::date)
  ...
  async replace(context: TransactionContext, input: ReplaceRouteStopsInput) {
    ...
    try {
      ...
      return this.list(context, route, input.effectiveDate);
    } catch (cause) {
  ...
  private translate(cause: unknown) {
    ...
    if (details.includes('route_stops_no_household_slot_overlap'))
      throw error('ROUTE_STOP_HOUSEHOLD_CONFLICT', 'Household already has an overlapping route stop', 409);
    if (details.includes('route_stops_no_sequence_overlap'))
      throw error('ROUTE_STOP_SEQUENCE_CONFLICT', 'Route stop sequence overlaps another plan', 409);
  }
